...
$(document).ready(function () {
	...
	$(".yellow_button").click(function () {
		var id = $(this).attr('id');
		var address = $(this).children("input[type='hidden']").val();
		window.location.href = "structure_lpu.php?struct_code="+id+"&address="+address;		
	});
...
<script type="text/javascript" src="static/js/js_userhelper.js"></script>
	...
	<div class="right_side" id="right_side">
		...
		<ul class="main_menu">
		{section loop=$STRUCTURES name=row}
			<li>	
				<div class="yellow_button" id="{$STRUCTURES[row].STR_CODE}">
					<span>{$STRUCTURES[row].TEXT}</span>
					<input type="hidden" value="{$STRUCTURES[row].ADDRESS}" />
				</div>
			</li>
		{/section}
		{if $MORELESS eq 1}
